...
## [0.7.6] - 2026-08-29（第三轮补漏：合并清单遗漏项 + 宿主漂移免疫）

> v0.7.5 发布后自查发现第一轮公平审计（deepseek）的 F5/F6 未进入修复清单（合并遗漏），补修；另修复宿主包漂移（全局 dsh 升级 0.1.2-alpha.2）导致的类型破坏。
...
- **新命令 /vrank（零子代理一键排名）**：手头已有 2-8 版候选（文本或文件路径）时，当前 agent 直接调 Verifier 出排名——N=2 用 compare、≥3 用 select（n_evaluations=1）、select flat 自动 compare 前二复核、复核仍在噪声带如实输出「候选并列」、unstable 呈现原始分数建议人工复核。纯函数层在 `src/vrank.ts`（零宿主依赖，CI core 可编译测试，7 个单测）；与 /bestofn 的分工：候选已存在 → /vrank，候选需要生成 → /bestofn
