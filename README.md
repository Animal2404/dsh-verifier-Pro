...
### /vrank 一键排名（v0.7.6+，零子代理）

手头已有 2-8 版候选（文本或文件），不派团队、当前 agent 直接调 Verifier 出排名：

```
/vrank 哪个方案更好 | 方案甲全文 | 方案乙全文        # 内联文本候选
/vrank 两份报告哪份更可信 | report-a.md | report-b.md  # 文件路径候选（自动读内容）
```

判别纪律内建：**2 个候选用 compare**（更便宜更有区分度），**≥3 个用 select**（`n_evaluations=1` 省成本）；select 信号 **flat 时自动 compare 前二复核**，复核仍在噪声带 → 如实输出「无可靠冠军，候选并列」；**unstable → 呈现原始分数并建议人工复核**，绝不编造排名。与 /bestofn 的分工：候选已在手用 /vrank（秒级出结果），候选需要**生成**用 /bestofn（开团队）。
