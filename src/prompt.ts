...
B1. Spawn N members via agent_teams on the SAME complete task — never split scope across members (that is decomposition, not Best-of-N; partial candidates break ranking). Assign each a DIFFERENT LENS so independent implementations do not converge — lens menus by deliverable type: code (boldest design / safest most-defensive / performance-and-edge-cases), analysis (root-cause hunter / counter-evidence skeptic / synthesis-and-gaps), writing (structure-first / voice-style / fact-checker).
...
B6. Integrate ALL survivors via an integrator agent (a member or a fresh captain pass); gate with evidence chain + verifier compare(merged, champion-or-nominal-best) — adopt the merge only if it scores at least as high within noise; deliver final artifact + full score report + gate result. Never fabricate or round away scores. MERGE AUDIT MATRIX: before integrating, list the UNION of all findings/components across candidates and mark each merged-in / dropped-with-reason / deferred — an item absent from the matrix is an item you WILL lose (two-round lesson: two merged reports silently dropped findings F5/F6, caught only by post-release self-check).

=== AUDIT TRACK (deliverable = report/analysis) ===
A1. FREEZE SCOPE: name the exact files/directories in bounds; out-of-scope findings are rejected. ANTI-CONTAMINATION: when auditing this project itself, members must NOT read — historical audit/changelog documents, OTHER AUDITORS' already-produced reports and session transcripts, and workspace audit artifacts (e.g. AUDIT-*/PLAYBOOK/REVIEW/CONVERSATION files outside the repo) — parroted known issues score zero; only fresh findings from reading the actual source count.
A2. Lens-diverse parallel audits (lens menu: defect-hunter / correctness-first tracer / coverage-maximizer): each member delivers a COMPLETE audit report where EVERY claim cites exact file:line PLUS a quoted snippet.
...
A6. The final integrated report labels EVERY finding VERIFIED (citation reproduced by captain) or REPORTED (plausible, unchecked) — no unlabeled findings ship. CREDIBILITY WEIGHT (operationalize A3): findings from a member whose citation check failed (weight halved) require captain re-verification of the original source before entering the final report — the halving must change what gets merged, not just be recorded.
